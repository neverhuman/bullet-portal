...
Chromium. `rendered`, `family`, and `packaged-farmd` remain additional local
integration requirements. They do not qualify authenticated Head execution;
Hub `just xbabe2-head` retains its separate admission and availability checks.
