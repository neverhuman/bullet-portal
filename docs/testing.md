...
| security | `bash scripts/ci-local.sh security` | `gitleaks detect`, a detector canary that fails the lane if gitleaks accepts a planted credential, the `CSRF_STORAGE_SLOT` symbol guard, `npm audit`, and `zizmor .`. Wrapper: `tools/security-lane.sh`; policy and pins: `agent/security-policy.toml`. |
