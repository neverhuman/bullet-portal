...
assert(
  read("ops/ci/fast.sh").includes(
    'assert-report.mjs vitest "$reports/vitest.json" 372',
  ),
  "exact 372-test Vitest count ratchet absent",
);
assert(
  read("ops/ci/fast.sh").includes(
...
assert(
  read("ops/ci/rendered.sh").includes(
    'assert-report.mjs junit "$reports/playwright.xml" 24',
  ),
  "exact 24-test mocked Playwright count ratchet absent",
);
assert(
  read("ops/ci/rendered.sh").includes(
...
assert(
  ...
  "retained original rendered Playwright inventory drifted",
);
assert(
